...
from brownfield.plugins.registry import get_handler, list_supported_languages
...
class LanguageDetector:
    ...
    def detect(self, project_root: Path) -> LanguageDetection:
        ...
        if not detections:
            raise RuntimeError(
                f"Could not detect primary language in {project_root}. Use --language to specify manually."
            )
