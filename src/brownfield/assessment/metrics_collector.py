...
class MetricsCollector:
    ...
    def collect(self, project_root: Path, language: str, mode: str = "quick") -> Metrics:
        """
    ...
    def _count_loc(self, project_root: Path) -> int:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                total = sum(lang_data.get("code", 0) for lang_data in data.values() if isinstance(lang_data, dict))
                return total
        except Exception:
            pass
    ...
    def _detect_coverage_source(self, project_root: Path) -> str:
        ...
        for item in project_root.iterdir():
            if (
                ...
                and item.name not in ["tests", "test", "docs", "examples"]
            ):
                return item.name
