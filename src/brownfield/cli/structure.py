...
def structure(verify: bool, output: str | None, format: str) -> None:
    ...
    try:
        ...
        if not state_path.exists():
            console.print("[red]✗ Error:[/red] Assessment required. Run 'brownfield assess' first.")
            raise click.Abort()
        ...
        if verify:
            ...
            if state.current_phase not in [Phase.STRUCTURE, Phase.ASSESSMENT]:
                console.print(
                    f"[yellow]⚠️  Warning:[/yellow] Already past structure phase (current: {state.current_phase.value})"
                )
