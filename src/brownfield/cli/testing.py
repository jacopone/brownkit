...
def testing(
    coverage_target: float, core_modules: tuple, skip_framework_install: bool, test_type: str
) -> None:
    ...
    try:
        ...
        if not state_path.exists():
            console.print(
                "[yellow]⚠[/yellow] No brownfield state found. "
        ...
        if state.current_phase not in [Phase.TESTING, Phase.STRUCTURE]:
            console.print(
                f"[yellow]⚠[/yellow] Current phase is {state.current_phase.name}. "
                "Expected STRUCTURE or TESTING phase."
            )
        ...
        if test_type in ["smoke", "both"]:
            ...
            console.print(f"  Generated {len(smoke_tests)} smoke test files")
        ...
        if test_type in ["contract", "both"]:
            ...
            console.print(f"  Generated {len(contract_tests)} contract test files")
        ...
        if not skip_framework_install:
            ...
            if result.dependencies_added:
                console.print(
                    f"[green]✓[/green] Dependencies added: {', '.join(result.dependencies_added)}"
                )
            ...
            if result.coverage >= coverage_target:
                console.print(
                    f"[green]✓[/green] Coverage target met ({coverage_target:.1%})"
                )
            else:
                console.print(
                    f"[yellow]⚠[/yellow] Coverage below target "
                    f"({result.coverage:.1%} < {coverage_target:.1%})"
                )
        else:
