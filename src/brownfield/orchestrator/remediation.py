...
class RemediationOrchestrator:
    ...
    def __init__(self, project_root: Path | None = None):
        ...
        # Note: Phase execution check happens in execute() since remediation phase is passed as parameter
    ...
    def execute(self, auto_commit: bool = True) -> RemediationResult:
        """Execute remediation for all aspects (structure, testing, quality).

        Args:
            auto_commit: Automatically commit changes after remediation completion

        Returns:
            RemediationResult with tasks completed/failed, commits, metrics
        """
        # Check if remediation phase can execute
        can_execute, error_msg = self.enforcer.can_execute_phase(WorkflowPhase.REMEDIATION)
        if not can_execute:
        ...
        # Mark remediation phase as started
        self.enforcer.mark_phase_started(WorkflowPhase.REMEDIATION)
        ...
        # Execute remediation for all aspects
        # 1. Structure remediation
        struct_completed, struct_failed = self._remediate_structure()
        tasks_completed.extend(struct_completed)
        tasks_failed.extend(struct_failed)
        if auto_commit and struct_completed:
            git_commits.append(self._commit_changes("refactor: Apply structure remediation"))

        # 2. Testing remediation
        test_completed, test_failed = self._remediate_testing()
        tasks_completed.extend(test_completed)
        tasks_failed.extend(test_failed)
        if auto_commit and test_completed:
            git_commits.append(self._commit_changes("test: Bootstrap test infrastructure"))

        # 3. Quality remediation
        quality_completed, quality_failed = self._remediate_quality()
        tasks_completed.extend(quality_completed)
        tasks_failed.extend(quality_failed)
        if auto_commit and quality_completed:
            git_commits.append(self._commit_changes("chore: Install quality gates"))

        # Collect metrics after remediation
        collector = MetricsCollector()
        metrics_after = collector.collect(self.project_root, self.lang_detection.language, mode="quick")

        # Update state
        self.state.current_metrics = metrics_after
        self.state.phase_timestamps["remediation_completed"] = datetime.utcnow()

        # Mark remediation phase as completed (only if all tasks succeeded)
        success = len(tasks_failed) == 0
        if success:
            self.enforcer.mark_phase_completed(WorkflowPhase.REMEDIATION)

        self.state_store.save(self.state)

        # Save checkpoint
        checkpoint_path = self._save_checkpoint(WorkflowPhase.REMEDIATION, tasks_completed, tasks_failed)
    ...
    def _save_checkpoint(self, phase: WorkflowPhase, tasks_completed: list[Task], tasks_failed: list[Task]) -> Path:
        ...
        """
        checkpoint_dir = self.project_root / ".specify/brownfield/checkpoints"
        ...
        checkpoint_data = {
            ...
            "tasks_completed": [{"task_id": t.task_id, "description": t.description} for t in tasks_completed],
            "tasks_failed": [{"task_id": t.task_id, "description": t.description} for t in tasks_failed],
        }
    ...
    def _get_next_phase(self, current_phase: Phase) -> Phase | None:
