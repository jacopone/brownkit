...
class ValidationOrchestrator:
    ...
    def execute(self) -> ValidationResult:
        ...
        current_metrics = collector.collect(
            self.project_root, lang_detection.language, mode="quick"
        )
        ...
        if all_passed and self.state.current_phase == Phase.VALIDATION:
            self.state.current_phase = Phase.GRADUATED
            self.state.graduated = True
            self.state.graduation_timestamp = datetime.utcnow()
    ...
    def _determine_recommended_phase(
    ...
    def _generate_report(
        self, gate_results: list[GateResult], all_passed: bool
    ) -> Path:
