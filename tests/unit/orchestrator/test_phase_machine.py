...
class TestRequirementChecking:
    ...
    def test_cannot_advance_to_testing_without_structure_complete(self, metrics_no_coverage):
...
class TestPhaseTransitionMatrix:
    ...
    def test_phase_transition_matrix(self, all_phase_states, from_phase, to_phase, should_succeed):
        ...
        if should_succeed:
            assert can_advance is True, f"Expected {from_phase.value} → {to_phase.value} to succeed, but got: {reason}"
        else:
            assert can_advance is False, f"Expected {from_phase.value} → {to_phase.value} to fail, but it succeeded"
